...
@Composable
fun AddNotes(modifier: Modifier = Modifier,
    ...
    Scaffold(
        ...
        floatingActionButton = {
            FloatingActionButton(onClick = {onNextButtonClicked},
                ...
                Icon(
                    Icons.Filled.Done, contentDescription = stringResource(
                    id = R.string.SaveNote
                )
                )
            }
        },
        content = {padding ->
            Column (modifier=modifier.padding(padding)){
                TextField(value=currentNoteText, onValueChange = {onNoteTextChange(it)},
                    modifier=modifier.fillMaxSize(),
                    colors = ExposedDropdownMenuDefaults.textFieldColors(textColor = Color(red=185, green = 233,blue=252) )
                )
            }
        }
    )
